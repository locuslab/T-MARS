...
import cv2
import torch
from dataset2metadata.text_detection.util.config import config as cfg
from dataset2metadata.text_detection.util.misc import fill_hole, regularize_sin_cos
from dataset2metadata.text_detection.util.misc import norm2, vector_cos, vector_sin
from dataset2metadata.text_detection.util.misc import disjoint_merge, merge_polygons
...
class TextDetector(object):
    ...
    def detect_contours(self, image, tr_pred, tcl_pred, sin_pred, cos_pred, radii_pred):
        ...
        :return:
            (list), tcl array: (N, n, 3), 3 denotes (x, y, radii)
        """
    ...
    def detect(self, image):
        ...
        for i in range(output.shape[0]):
            image_i = image[i].data.cpu().numpy()
            tr_pred = output[i,0:2].softmax(dim=0).data.cpu().numpy()
            tcl_pred = output[i,2:4].softmax(dim=0).data.cpu().numpy()
            sin_pred = output[i,4].data.cpu().numpy()
            cos_pred = output[i,5].data.cpu().numpy()
            radii_pred = output[i,6].data.cpu().numpy()
            try:
                contours = self.detect_contours(image_i, tr_pred, tcl_pred, sin_pred, cos_pred, radii_pred)  # (n_tcl, 3)
            except:
                contours = []
                print("Contour Error Happened, returned no text detection")
            ...
            all_ouptuts.append(dict_output)

        # return all_contours, all_ouptuts
        return all_contours, all_ouptuts
